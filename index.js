...
let margin = {top: 60, right: 16, bottom: 60, left: 60}

// var years = object.data.map((item) => {
...
.domain(data.map((d) => d[0].substring(0, 4)))
.range([margin.left, widths - margin.right])



const yScale = d3.scaleLinear()
      .domain([0, d3.max(data, (d) => d[1])]).nice()
      .range([heights - margin.bottom, margin.right]);
...
svg.append("g")
.attr("transform", "translate(0, " + (heights - margin.right) + ")")
.call(xAxis)
svg.append("g")
.attr("transform", "translate(" + margin.left + ",0)")
.call(yAxis)
// creating a RECT inside the SVG
...
.append("rect")
.attr("class", "bar")
.attr('x', (d)=> xScale(d[0].substring(0, 4)))
  .attr('y', (d)=> yScale(d[1]))
  .attr("width", xScale.bandwidth())
  .attr('height', (d)=> heights - yScale(d[1]))
